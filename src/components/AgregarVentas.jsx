...
class AgregarVentas extends Component{
    constructor(props) {
      ...
      addSell(e) {
        e.preventDefault();
        if(this.state._id) {
          fetch(`https://mercadoscampesinos.herokuapp.com/api/ventas/${this.state._id}`, {
            method: 'PUT',
            body: JSON.stringify({
          ...
          fetch('https://mercadoscampesinos.herokuapp.com/api/ventas', {
            method: 'POST',
      ...
      editSell(id) {
        fetch(`https://mercadoscampesinos.herokuapp.com/api/ventas/${id}`)
          .then(res => res.json())
          .then(data => {
            console.log(data);
            this.setState({
